...
export default function EditJobForm({ job }: { job:Job }) {
  const { register, handleSubmit, control, reset, formState: { errors } } = useForm({
    defaultValues: {
      ...
      tags: job?.tags.map(tag => ({ label: tag, value: tag })) || [],
      publish_date: job?.publishdate || getTodayDate(),
    ...
    <div className="min-h-screen">
      <div className="max-w-6xl w-full space-y-8">
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6 bg-white p-8 rounded shadow">
          ...
          {/* Submission Status */}
          {submitStatus === 'error' && (
            <p className="mt-2 text-center text-red-600">Failed to update the job. Please try again.</p>
          )}
          {submitStatus === 'success' && (
            <p className="mt-2 text-center text-green-600">Job updated successfully!</p>
          )}
        </form>
      </div>
    </div>
